...
LEFT JOIN LATERAL (
    SELECT
        SUM(agg.count) AS total_interactions,
        SUM(agg.count) FILTER (WHERE agg.interaction_type = 'deploy') AS deployment_count,
        SUM(agg.count) FILTER (WHERE agg.interaction_type = 'publish_failed') AS error_count,
