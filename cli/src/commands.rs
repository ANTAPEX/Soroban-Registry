...
pub async fn search(
    ...
    if explain {
        let results: Vec<crate::ranking::ExplainSearchResult> = items
            .iter()
            .map(|contract| {
                let (score, factors) = crate::ranking::calculate_ranking_factors(query, contract);
                crate::ranking::ExplainSearchResult {
        ...
        if json {
            let response = crate::ranking::ExplainSearchResponse { results };
            println!("{}", serde_json::to_string_pretty(&response)?);
            return Ok(());
        }

        println!("\n{}", "Search Ranking Transparency & Explainability (--explain):".bold().cyan());
        println!("{}", "=".repeat(80).cyan());
        println!(
        ...
        for (idx, contract) in items.iter().enumerate() {
            ...
            println!("   Total Score: {}", format!("{:.2}", score).bold().yellow());
            println!(
            ...
            println!(
                ...
                if contract.is_verified { "verified contract" } else { "unverified" }
            );
            println!(
            ...
            println!(
                ...
                if factors.deprecation_penalty < 0.0 { "deprecated" } else { "active" }
            );
            println!();
        }
