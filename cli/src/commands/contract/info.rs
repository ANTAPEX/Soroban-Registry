use crate::support::output_format::{self, OutputFormat};
...
pub async fn list_contracts(
    ...
    let (status, body) = crate::support::cached_http::cached_get(&url, &query)
        .await
...
pub async fn info(api_url: &str, id: &str, json_output: bool) -> Result<()> {
    ...
    let data: serde_json::Value = crate::support::registry::client(api_url)
        .await?
        .send_json(
...
pub async fn run_details(
    ...
    let contract: serde_json::Value = crate::support::registry::client(api_url)
        .await?
        .send_json(
