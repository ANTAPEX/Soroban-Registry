...
use crate::commands::contract::deploy::{upload_icon_to_backend, validate_and_process_icon};
...
pub async fn run(args: UpdateArgs<'_>) -> Result<()> {
    ...
    let updated = crate::support::registry::uncached_client(args.api_url)
        .await?
...
async fn fetch_contract(api_url: &str, address: &str) -> Result<ContractRecord> {
    let response = crate::support::registry::uncached_client(api_url)
        .await?
