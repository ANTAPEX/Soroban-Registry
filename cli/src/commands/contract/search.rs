...
//!
//! The flag-to-request logic lives in [`crate::support::search_pagination`].

use anyhow::{anyhow, Context, Result};
use colored::Colorize;
use registry_client::{ContractHit, PageCollection, PageCursor, PaginationMode, StopReason};

use crate::support::search_pagination::{build_limits, build_request, resolve_mode, SearchOptions};
...
pub async fn run(api_url: &str, options: SearchOptions) -> Result<()> {
    ...
    let mut walk = crate::support::registry::uncached_client(api_url)
        .await?
