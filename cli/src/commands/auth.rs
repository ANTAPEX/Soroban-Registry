...
fn prompt_identity(method: AuthMethod) -> Result<String> {
    ...
    let value = crate::commands::wizard::prompt_with_validation(
        label,
        None::<String>,
        |value| match method {
...
fn prompt_secret(method: AuthMethod) -> Result<String> {
    ...
    crate::commands::wizard::prompt_with_validation(
        label,
        None::<String>,
...
fn load_legacy_api_key() -> Result<Option<(String, Vec<String>)>> {
    let config = crate::config::user::load().context("Failed to read user config")?;
