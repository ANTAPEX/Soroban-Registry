...
///
/// Keys are composed exactly as [`crate::commands::cache::http_cache_key`] composes them,
/// so a response fetched through the client and one fetched through
/// [`crate::support::cached_http`] share a cache entry.
struct CliResponseCache {
    options: crate::support::cached_http::HttpCacheOptions,
}
...
impl ResponseCache for CliResponseCache {
    fn get(&self, key: &str) -> Option<String> {
        ...
        match crate::commands::cache::get_http_entry(&Self::disk_key(key)) {
            Ok(Some(entry)) => {
                if self.options.verbose >= 1 {
                    eprintln!(
                        "{} hit (expires in {}s): {}",
    ...
    fn put(&self, key: &str, body: &str) {
        ...
        if let Err(err) = crate::commands::cache::set_http_entry(&Self::disk_key(key), body) {
...
pub async fn client(api_url: &str) -> Result<RegistryClient> {
    let token = crate::commands::auth::access_token_for_requests(api_url).await?;
    let cache = Arc::new(CliResponseCache {
        options: crate::support::cached_http::cache_options(),
    });
...
mod tests {
    ...
    fn cache_keys_match_the_cli_http_cache_scheme() {
        ...
        assert_eq!(
            ...
            crate::commands::cache::http_cache_key(
